...
listen st =
  ...
  do ctx <- baselineContextSSL
     dbType <- DB.lookup "" st -- hack to be able to get fbCtx from FireState t
     let loc = (fbCtx dbType) ++ ".json?auth="
      ...
      case eItm of
        Left e -> do
          logD $
            cs $
            "Error (" <> (cs $ show e) <>
            ") when performing request for updated data for " <>
            (show evnt) <>
            "." -- log
          return $
            INVALID $
            "Unable to fetch updated data for item: " <> (cs $ show loc)
        Right itm ->
          return $
          evnt
          { item = Just itm
